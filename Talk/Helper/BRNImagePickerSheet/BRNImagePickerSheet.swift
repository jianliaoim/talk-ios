...
class BRNImagePickerSheet: UIView, UITableViewDataSource, UITableViewDelegate, UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout {
    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        if buttonIndex == self.cancelButtonIndex {
            buttonTitle = NSLocalizedString("Cancel", tableName: nil, bundle: JLInternationalManager.currentLanguageBundle(), value: "", comment: "Cancel")
        }
        else {
    ...
    func showInView(view: UIView) {
        ...
        UIView.animateWithDuration(BRNImagePickerSheet.presentationAnimationDuration, delay: 0.0, usingSpringWithDamping: 1.0, initialSpringVelocity: 0.0, options: UIViewAnimationOptions.LayoutSubviews, animations: { () -> Void in
    ...
    private func fetchAssets() {
        ...
        let collectionResult: PHFetchResult = PHAssetCollection.fetchAssetCollectionsWithType(PHAssetCollectionType.SmartAlbum, subtype: PHAssetCollectionSubtype.SmartAlbumRecentlyAdded, options: nil)
        let collection = collectionResult.lastObject as! PHCollection
        if collection.isKindOfClass(PHAssetCollection) {
            let result = PHAsset.fetchAssetsInAssetCollection(collection as! PHAssetCollection, options: options)
    ...
    internal func getSelectedImagesWithCompletion(completion: (images:[UIImage?]) -> Void) {
        var images = [UIImage?]()
