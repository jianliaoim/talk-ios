...
- (void)viewDidLoad {
    ...
    _pagingScrollView.contentSize = [self contentSizeForPagingScrollViewWithNumber:[self numberOfPhotos]];
...
- (void)performLayout {
    ...
    DDLogDebug(@"numberOfPhotos:%ld", numberOfPhotos);
...
//    }
    ...
    if (_displayNavArrows) {
        [items addObject:[[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"icon-browser-star"] style:UIBarButtonItemStylePlain target:self action:@selector(starAction)]];
        [items addObject:flexSpace];
//        [items addObject:[[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"icon-photo-tag"] style:UIBarButtonItemStylePlain target:self action:@selector(showGridAnimated)]];
//        [items addObject:flexSpace];
//        [items addObject:[[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"icon-photo-trash"] style:UIBarButtonItemStylePlain target:self action:@selector(deleteAction)]];
    }
...
- (void)layoutVisiblePages {
	...
	_pagingScrollView.contentSize = [self contentSizeForPagingScrollViewWithNumber:(int)[self numberOfPhotos]];
...
- (CGSize)contentSizeForPagingScrollViewWithNumber:(unsigned long)number {
    // We have to use the paging scroll view's bounds to calculate the contentSize, for the same reason outlined above.
    CGRect bounds = _pagingScrollView.bounds;
    return CGSizeMake(bounds.size.width * number, bounds.size.height);
}
...
- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView {
    ...
    //reload the photos-browser
    if (_currentPageIndex == [self numberOfPhotos] - 1 && _needToReload) {
        [self reloadData];
        _needToReload = FALSE;
    }
}
...
- (void)setContenSizeWithNumber:(unsigned long)number {
    _pagingScrollView.contentSize = [self contentSizeForPagingScrollViewWithNumber:number];
}
...
- (void)updateToolBar {
    ...
    if (photo.isFavorite) {
        starImage = [[UIImage imageNamed:@"icon-browser-star-yellow"] imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal];
    } else {
        starImage = [UIImage imageNamed:@"icon-browser-star"];
    }
    UIBarButtonItem *starItem = [[UIBarButtonItem alloc] initWithImage:starImage style:UIBarButtonItemStylePlain target:self action:@selector(starAction)];
    [items addObject:starItem];
    [items addObject:flexSpace];
    [items addObject:[[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"icon-broswer-tag"] style:UIBarButtonItemStylePlain target:self action:@selector(tagAction)]];
    [items addObject:flexSpace];
    [items addObject:[[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"icon-forward"] style:UIBarButtonItemStylePlain target:self action:@selector(forwardAction)]];
    if (photo.creatorID) {
        NSString *currentID = [[NSUserDefaults standardUserDefaults] objectForKey:kCurrentUserKey];
        if ([photo.creatorID isEqualToString:currentID] ||  [TBUtility isManagerForCurrentAccount]) {
        [items addObject:flexSpace];
        [items addObject:[[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"icon-photo-trash"] style:UIBarButtonItemStylePlain target:self action:@selector(deleteAction)]];
        }
    }
...
#pragma mark - Custom Star action and delete action

- (void)starAction {
    if ([self.delegate respondsToSelector:@selector(photoBrowser:starPhotoAtIndex:)]) {
        [self.delegate photoBrowser:self starPhotoAtIndex:_currentPageIndex];
        MWPhoto *currentPhoto = [self photoAtIndex:_currentPageIndex];
        currentPhoto.isFavorite = !currentPhoto.isFavorite;
        [self updateToolBar];
    }
}
...
- (void)tagAction {
    if ([self.delegate respondsToSelector:@selector(photoBrowser:starPhotoAtIndex:)]) {
        [self.delegate photoBrowser:self tagPhotoAtIndex:_currentPageIndex];
    }
}

- (void)forwardAction {
    if ([self.delegate respondsToSelector:@selector(photoBrowser:starPhotoAtIndex:)]) {
        [self.delegate photoBrowser:self forwardPhototAtIndex:_currentPageIndex];
    }
}
...
- (void)deleteAction {
    ...
    [actionSheet SH_addButtonDestructiveWithTitle:NSLocalizedString(@"Delete", @"Delete") withBlock:^(NSInteger theButtonIndex) {
        if ([self.delegate respondsToSelector:@selector(photoBrowser:deletePhototAtIndex:)]) {
            [self.delegate photoBrowser:self deletePhototAtIndex:_currentPageIndex];
        }
    }];
